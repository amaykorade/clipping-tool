...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 dark:border-slate-700 dark:bg-slate-900/95 dark:supports-[backdrop-filter]:bg-slate-900/80">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-6 px-4 sm:px-6">
        ...
        <div className="hidden items-center gap-3 md:flex">
          <ThemeToggle />
          {status === "loading" ? (
            <span className="h-9 w-9 animate-pulse rounded-full bg-slate-200 dark:bg-slate-700" />
          ) : session?.user ? (
            <div className="relative" ref={accountRef}>
              <button
                type="button"
                onClick={() => setAccountOpen((o) => !o)}
                className="rounded-full ring-1 ring-slate-200 transition hover:ring-slate-300 hover:bg-slate-50 dark:ring-slate-600 dark:hover:ring-slate-500 dark:hover:bg-slate-800"
              ...
              {accountOpen && (
                <div
                  className="absolute right-0 top-full z-50 mt-1.5 min-w-[200px] rounded-xl border border-slate-200 bg-white py-1.5 shadow-lg dark:border-slate-600 dark:bg-slate-800"
                  role="menu"
                >
        ...
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-lg text-slate-600 transition hover:bg-slate-100 md:hidden dark:text-slate-300 dark:hover:bg-slate-800"
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="border-t border-slate-200 bg-white px-4 py-4 md:hidden dark:border-slate-700 dark:bg-slate-900">
          ...
          <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 dark:border-slate-700">
            {status === "loading" ? (
              <span className="block h-10 w-10 rounded-full bg-slate-100 dark:bg-slate-700" />
            ) : session?.user ? (
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <button
                    ...
                    className="flex items-center gap-3 rounded-lg p-1 transition hover:bg-slate-50 dark:hover:bg-slate-800"
                ...
                {mobileAccountOpen && (
                  <div className="mt-2 rounded-xl border border-slate-200 bg-white py-1 dark:border-slate-700 dark:bg-slate-800">
                    <Link
                      href="/account"
                    ...
                    <button
                      type="button"
                      onClick={() => { signOut(); setMobileOpen(false); setMobileAccountOpen(false); }}
                      className="flex w-full items-center gap-2 px-3 py-2.5 text-sm text-slate-600 hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-slate-700"
                    >
                      <SignOutIcon />
                      Sign out
                    </button>
